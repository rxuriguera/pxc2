...
public class Node {
	...
	public List<PeerMessage> sendToPeer(String peerid, String msgtype,
		...
		return this.sendToPeer(peerid, new PeerMessage(msgtype, msgdata),
				waitreply);
	}

	public List<PeerMessage> sendToPeer(String peerid, PeerMessage message,
			boolean waitreply) {
		PeerInfo pd = null;
		if (router != null)
		...
		if (pd == null) {
			LoggerUtil.getLogger().severe(
					String.format("Unable to route %s to %s", message
							.getMsgType(), peerid));
			return new ArrayList<PeerMessage>();
		}

		return connectAndSend(pd, message, waitreply);
	}
	...
	public List<PeerMessage> connectAndSend(PeerInfo pd, String msgtype,
			String msgdata, boolean waitreply) {
		return this.connectAndSend(pd, new PeerMessage(msgtype, msgdata),
				waitreply);
	}

	public List<PeerMessage> connectAndSend(PeerInfo pd, PeerMessage tosend,
			boolean waitreply) {
	...
	public void mainLoop() {
		try {
			...
			LoggerUtil.getLogger().severe(
					"Stopping main loop (SocketExc): " + e);
		} catch (IOException e) {
