...
public class XmlMusicLibrary {
		...
		if (terms.length > 0){
			...
			while (it.hasNext()) {
				...
				for (int i = 0; i<terms.length; ++i){
					boolean temp = false;
					if (temporal.getChild("title").getText().contains(terms[i])) temp=true;
					else if (temporal.getChild("album").getText().contains(terms[i])) temp=true;
					else if (temporal.getChild("artist").getText().contains(terms[i])) temp=true;
					ok=(ok && temp);
				}
	...
	private Element fileToElement(String path) throws Exception {
		...
		if (songFile.exists() && songFile.canRead()){
			if (path.endsWith(".mp3")){
				String hash=Integer.toString(songFile.hashCode());
				if (!existSong(hash)){
					MP3File  mp3 = new MP3File(songFile);
					if (mp3.hasID3v1Tag()){
						ID3v1 tag = mp3.getID3v1Tag();
						song.addContent(new Element("title").addContent(tag.getTitle()));
						song.addContent(new Element("artist").addContent(tag.getArtist()));
						song.addContent(new Element("album").addContent(tag.getAlbum()));
					}else if (mp3.hasID3v2Tag()){
						AbstractID3v2 tag = mp3.getID3v2Tag();
						song.addContent(new Element("title").addContent(tag.getSongTitle()));
						song.addContent(new Element("artist").addContent(tag.getLeadArtist()));
						song.addContent(new Element("album").addContent(tag.getAlbumTitle()));
					}else return null;
				
					song.addContent(new Element("hash").addContent(hash));
					song.addContent(new Element("path").addContent(songFile.getAbsolutePath()));
	...
	private boolean existSong(String hash) {
		...
		while (it.hasNext()) {
			temporal = (Element) it.next();
			if (temporal.getChild("hash").getText().equals(hash)) return true;
		}
